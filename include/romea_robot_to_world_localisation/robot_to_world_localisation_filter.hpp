#ifndef __R2WLocalisationFilter_HPP__
#define __R2WLocalisationFilter_HPP__
...
public :
  ...
  using UpdaterInterfaceLinearSpeed = LocalisationUpdaterInterface<Filter,UpdaterLinearSpeed,romea_localisation_msgs::msg::ObservationTwist2DStamped>;
  using UpdaterInterfaceLinearSpeeds = LocalisationUpdaterInterface<Filter,UpdaterLinearSpeeds,romea_localisation_msgs::msg::ObservationTwist2DStamped>;
  using UpdaterInterfaceAngularSpeed = LocalisationUpdaterInterface<Filter,UpdaterAngularSpeed,romea_localisation_msgs::msg::ObservationAngularSpeedStamped>;
  using UpdaterInterfaceAttitude = LocalisationUpdaterInterface<Filter,UpdaterAttitude,romea_localisation_msgs::msg::ObservationAttitudeStamped>;
  using UpdaterInterfaceCourse = LocalisationUpdaterInterface<Filter,UpdaterCourse,romea_localisation_msgs::msg::ObservationCourseStamped>;
  using UpdaterInterfacePosition = LocalisationUpdaterInterface<Filter,UpdaterPosition,romea_localisation_msgs::msg::ObservationPosition2DStamped>;
  using UpdaterInterfaceRange = LocalisationUpdaterInterface<Filter,UpdaterRange,romea_localisation_msgs::msg::ObservationRangeStamped>;
  using UpdaterInterfacePose = LocalisationUpdaterInterface<Filter,UpdaterPose,romea_localisation_msgs::msg::ObservationPose2DStamped>;


public :

  R2WLocalisationFilter(std::shared_ptr<rclcpp::Node> node);
...
private :
  ...
  void add_proprioceptive_updater_interface_(std::shared_ptr<rclcpp::Node> node,
                                             const std::string &updater_name);

  template <typename interface>
  void add_exteroceptive_updater_interface_(std::shared_ptr<rclcpp::Node> node,
                                            const std::string &updater_name);
  ...
  updater_interfaces_()
{
  ...
  add_proprioceptive_updater_interface_<UpdaterInterfaceLinearSpeeds>(node,"linear_speeds_updater");
  add_proprioceptive_updater_interface_<UpdaterInterfaceAngularSpeed>(node,"angular_speed_updater");
...
//-----------------------------------------------------------------------------
template <FilterType FilterType_>
template <typename Interface> void R2WLocalisationFilter<FilterType_>::
add_proprioceptive_updater_interface_(std::shared_ptr<rclcpp::Node> node,
                                      const std::string &updater_name)
{
  ...
  if(!get_updater_topic_name(node,updater_name).empty())
  {
    ...
    auto updater = make_proprioceptive_updater<Updater>(node,
                                                        updater_name);

    auto plugin = make_updater_interface<Interface>(node,
                                                    updater_name,
                                                    filter_,
                                                    std::move(updater));
...
template <FilterType FilterType_>
template <typename Interface> void R2WLocalisationFilter<FilterType_>::
add_exteroceptive_updater_interface_(std::shared_ptr<rclcpp::Node> node,
                                     const std::string &updater_name)
{
  ...
  if(!get_updater_topic_name(node,updater_name).empty())
  {
    ...
    using Updater = typename Interface::Updater;
    auto updater = make_exteroceptive_updater<Updater,FilterType_>(node,
                                                                   updater_name);
    auto plugin = make_updater_interface<Interface>(node,
                                                    updater_name,
                                                    filter_,
